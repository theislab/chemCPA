...
val_idx = sc.pp.subsample(
    adata_sciplex[validation_cond], 0.4, copy=True, random_state=42
).obs.index
...
val_idx = sc.pp.subsample(
    adata_sciplex[validation_cond], 0.2, copy=True, random_state=42
).obs.index
adata_sciplex.obs.loc[val_idx, "split_ood_finetuning"] = "test"

validation_cond = adata_sciplex.obs.split_ood_finetuning == "train"
val_idx = sc.pp.subsample(
    adata_sciplex[validation_cond], 0.04, copy=True, random_state=42
).obs.index
adata_sciplex.obs.loc[val_idx, "split_ood_finetuning"] = "test"
...
val_idx = sc.pp.subsample(
    adata_sciplex[validation_cond], 0.05, copy=True, random_state=42
).obs.index
adata_sciplex.obs.loc[val_idx, "split_ood_finetuning"] = "test"
...
if "split_ho_epigenetic" not in list(adata_sciplex.obs):
    ...
    obs_train, obs_val = train_test_split(
        adata_sciplex.obs.index, test_size=0.05, random_state=42
    )
...
if "split_ho_epigenetic_all" not in list(adata_sciplex.obs):
    ...
    val_idx = sc.pp.subsample(
        adata_sciplex[validation_cond], 0.5, copy=True, random_state=42
    ).obs.index
...
if "split_random" not in list(adata_sciplex.obs):
    ...
    obs_train, obs_tmp = train_test_split(
        adata_sciplex.obs.index, test_size=0.3, random_state=42
    )
...
# %%
sc.write(
    PROJECT_DIR / "datasets" / "sciplex_complete_lincs_genes_v2.h5ad",
...
# %%
sc.write(
    PROJECT_DIR / "datasets" / "sciplex_complete_subset_lincs_genes_v2.h5ad",
    adata_subset,
)
...
# %%
sc.write(
    PROJECT_DIR / "datasets" / "sciplex_complete_middle_subset_lincs_genes_v2.h5ad",
    adata_lincs_genes,
)
